## Function to evaluate Change in Chi2 before and after adjustment for a covariate
...
lrtest_change_chi2 <- function(dataset, 
  ...
  stratify_vars <- stratify_vars[!stratify_vars %in% c(confounder_vars, adjustment_vars)]
  ...
  for (j in seq_along(adjustment_vars)) {
    ...
    if(class(dataset[[ adjustment_vars[j] ]]) == "character"){
      ...
      get_char_list[[ adjustment_vars[j] ]] <- adjustment_vars[j]
  ...
  if(length(get_char_list) > 0) {  
      ...
      adjustment_vars <- adjustment_vars[!adjustment_vars %in% get_char]
      # Get adjustment vars for confounder/mediation model 
      
        
      adjust <- paste(
          paste(c(adjustment_vars), collapse = " + "),
          paste0(" + as.factor(", get_char, ") ", collapse = "")
      )
  
  }
  
  adjust <- paste(adjustment_vars, collapse = " + ")
